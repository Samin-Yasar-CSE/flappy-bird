...
function initializeGame() {
    // CHANGED: Dynamically start the plane in the vertical center of the screen
...
function updateBird() {
    ...
    birdY += velocity;
    bird.style.top = `${birdY}px`;
    // CHANGED: Use dynamic container height instead of hardcoded 600
    if (birdY < 0 || birdY > gameContainer.offsetHeight) gameOver();
}

function createObstacle() {
    const containerHeight = gameContainer.offsetHeight;

    // Prevent maxTopY from becoming negative on very small screens in landscape
    const maxTopY = Math.max(20, containerHeight - gapSize - 180);
...
// ADDED: Mobile touch control. Tapping the screen makes the plane jump.
gameContainer.addEventListener('touchstart', e => {
    e.preventDefault();
    jump();
}, { passive: false });

// ADDED: Mouse click control (Great for testing without a keyboard)
